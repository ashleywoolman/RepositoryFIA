...
table(nmcond3$SUCP) #show many with 3 or 4 entries
...
uttree1<- uttree2000 %>% mutate(cycle_aw = case_when(CYCLE==2 ~ '1',
                                                     CYCLE==3 ~ '2'))
cotree2000<-read.csv("COtree2000.csv")
cotree1<- cotree2000 %>% mutate(cycle_aw = case_when(CYCLE==2 ~ '1',
                                                     CYCLE==3 ~ '2'))
aztree2000<-read.csv("AZtree2000.csv")
aztree1<- aztree2000 %>% mutate(cycle_aw = case_when(CYCLE==3 ~ '1',
                                                     CYCLE==4 ~ '2'))
nmtree2000<-read.csv("NMtree2000.csv")
nmtree1<- nmtree2000 %>% mutate(cycle_aw = case_when(CYCLE==3 ~ '1',
                                                     CYCLE==4 ~ '2'))
treesfc<- rbind(uttree1, nmtree1, cotree1, aztree1)
treesfc$cycle_aw<- factor(treesfc$cycle_aw)
summary(treesfc) # cycle 1: 368504, 2: 267905  
...
length(unique(treesfc$SUCP)) # 14,642
...
length(unique(treesfc4$SUCP)) # total 9893 plots
summary(treesfc4)

#check
chkt1<- treesfc4 %>% filter(cycle_aw==1)
length(unique(chkt1$SUCP)) # 9116
chkt2<- treesfc4 %>% filter(cycle_aw==2)
length(unique(chkt2$SUCP)) # 9893
# there are 777 more in cycle 2.............................
...
length(unique(treesfcnotrtmt$SUCP)) #9547 plots

# 9893- 9547= 346 plots removed. (not 360... some already filtered out?)
...
length(unique(pipoplots2$SUCP)) #1520 plots
#check
pipock<- pipoplots2 %>% filter(cycle_aw==1)
length(unique(pipock$SUCP)) # 1470
pipock2<- pipoplots2 %>% filter(cycle_aw==2)
length(unique(pipock2$SUCP)) # 1520
# 50 more in cycle 2 than 1
